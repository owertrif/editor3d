...
std::vector<GLuint> Model::getIndices(json accessor) {
	...
	else throw std::invalid_argument("Type is invalid = " + std::to_string(componentType));
	return indices;
}
...
std::vector<glm::vec2> Model::groupFloatVec2(std::vector<float> floatVec) {
	std::vector <glm::vec2> floatVec2;

	for (unsigned int i = 0; i < floatVec.size(); i) {
		floatVec2.push_back(glm::vec2(floatVec[i++], floatVec[i++]));
	}
	return floatVec2;
}

std::vector<glm::vec3> Model::groupFloatVec3(std::vector<float> floatVec) {
	std::vector <glm::vec3> floatVec3;

	for (unsigned int i = 0; i < floatVec.size(); i) {
		floatVec3.push_back(glm::vec3(floatVec[i++], floatVec[i++], floatVec[i++]));
	}
	return floatVec3;
}

std::vector<glm::vec4> Model::groupFloatVec4(std::vector<float> floatVec) {
	std::vector <glm::vec4> floatVec4;

	for (unsigned int i = 0; i < floatVec.size(); i) {
		floatVec4.push_back(glm::vec4(floatVec[i++], floatVec[i++], floatVec[i++], floatVec[i++]));
	}
	return floatVec4;
}
...
void Model::loadMeshes(unsigned int indMeshes) {
	unsigned int posAccInd = JSON["meshes"][indMeshes]["primitives"][0]["attributes"]["POSITION"];
	unsigned int normalAccInd = JSON["meshes"][indMeshes]["primitives"][0]["attributes"]["NORMAL"];
	unsigned int texAccInd = JSON["meshes"][indMeshes]["primitives"][0]["attributes"]["TEXCOORD_0"];
	unsigned int indAccInd = JSON["meshes"][indMeshes]["primitives"][0]["indices"];
